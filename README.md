...
ui.to(3).nextSlot().progress(72, "Battery", 100, "#36C36B", "72%");
ui.to(3).currentSlot().progress(56, "Battery", 100, "#36C36B", "56%"); // обновит тот же slot на месте
...
## Slot И Обновление Виджетов На Месте

Android теперь умеет закрепленные slot-позиции в верхней части terminal.
Это удобно, когда нужно не добавлять новый виджет в конец лога, а обновлять уже существующий элемент на месте.

На стороне C++ теперь есть два удобных режима:

- `nextSlot()` — открыть следующий auto-slot и запомнить его как текущий
- `currentSlot()` — отправить следующий виджет в уже текущий slot этого terminal

Это самый удобный вариант, когда номер slot не хочется помнить руками:
...
- первая команда создаст новый auto-slot, например `slot=0`
- вторая команда не добавит новую строку, а обновит этот же текущий slot

Если нужен точный контроль, остается и явный режим:

```cpp
ui.to(3).at(5).progress(72, "Battery", 100, "#36C36B", "72%");
ui.to(3).at(5).progress(56, "Battery", 100, "#36C36B", "56%");
```

Есть и алиас `lastSlot()`, если удобнее мыслить как "обновить прошлый slot":

```cpp
ui.to(3).lastSlot().progress(60, "Battery", 100, "#36C36B", "60%");
```

Сырая строка выглядит так:

```text
@3 ui type=progress label="Battery" value=56 max=100 fill=#36C36B display="56%" slot=0
```

На Android также понимаются алиасы `index`, `pos` и `position`, но библиотека `TimberWidget`
для единообразия отправляет именно `slot=...`.
...
| `at(...)` | Временно выбирает slot для следующего виджета | `ui.at(0).progress(72, "Battery", 100, "#36C36B", "72%");` |
| `nextSlot()` | Открывает следующий auto-slot в terminal | `ui.nextSlot().progress(72, "Battery", 100, "#36C36B", "72%");` |
| `currentSlot()` | Обновляет текущий slot terminal | `ui.currentSlot().progress(56, "Battery", 100, "#36C36B", "56%");` |
| `lastSlot()` | Алиас для текущего slot | `ui.lastSlot().progress(60, "Battery", 100, "#36C36B", "60%");` |
